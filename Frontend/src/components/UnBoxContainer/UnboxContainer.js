...
import './UnboxContainer.css';
import box from '../../images/unbox/box.jpg';
import boxvip from '../../images/unbox/boxvip.jpg';
...
function UnboxContainer() {
    ...
    const preUnbox = async () => {
        const nameOfPlant = document.getElementById('name_of_box').value;
        for(let i = 0 ;i< nameOfPlant.length ;i++) {
            if(nameOfPlant[i] == ' ') {
                alert("name does not contain whitespace");
                return;
            }
        }
        if(nameOfPlant.length < 4 || nameOfPlant.length > 15) {
            alert("name should from 4 to 15 character")
            return;
        }
        openModal();
    }
    const preUnboxVip = async () => {
        const nameOfPlant = document.getElementById('name_of_box_vip').value;
        for(let i = 0 ;i< nameOfPlant.length ;i++) {
            if(nameOfPlant[i] == ' ') {
                alert("name does not contain whitespace");
                return;
    ...
    const UnBox = async ()=> {
        ...
        try {
            ...
            const txn_test = await pvzWithSigner.unBox(false, nameOfPlant);
    ...
    const UnBoxVip = async () => {
        const nameOfPlant = document.getElementById('name_of_box_vip').value;
        const signer =  provider.getSigner();
        const pvzWithSigner = plantVsZombiesNFTContract.connect(signer);
        try {
            setTexAmount("pending");
            const txn_test = await pvzWithSigner.unBox(true, nameOfPlant);
            const haha = setInterval(async function() {
                const txReceipt = await provider.getTransactionReceipt(txn_test.hash);
                console.log("setInterval");
                if (txReceipt && txReceipt.blockNumber) {
    ...
    return (
        <div className='unBoxContainer'>
            <div className='background'></div>
            <div className='unBoxContent'>
                <div className='box'>
                    <img className='imgbox_unbox' src = {box}></img>
                    <div className='content-box'>
                        <div className='title-box'>Box</div>
                        <div className='amount_data_user'>
                            <div className='text-content'>Amount: </div>
                            <div className='unboxNumber'>{amountBox}</div>
                        </div> 
                        <div className='input_name_content'>
                            <label for="name_of_box" className='label_name_box'>Name :</label>
                            <input type="text" id="name_of_box" className="input_name_box" placeholder=" Plant's name" maxlength = "15" pattern="[a-z]{1,15}"/>
                        </div>
                        <button className='buttonUnBox' onClick={preUnbox}>UnBox</button>
                    </div>
                </div>
                <div className='boxvip'>
                    <img className='imgboxvip' src = {boxvip}></img>    
                    <div className='content-boxvip'>
                        <div className='title-boxvip'>Box Vip</div>
                        <div className='amount-data'>
                            <div className='text-content'>Amount: </div>
                            <div className='unboxNumber'>{amountBoxVip}</div>
                        </div>
                        <div className='input_name_content'>
                            <label for="name_of_box_vip" className='label_name_box'>Name :</label>
                            <input type="text" id="name_of_box_vip" className="input_name_box input_name_box_vip" placeholder=" Plant's name" maxlength = "20"/>
                        </div>
                        <button className='buttonUnBoxVip' onClick={preUnboxVip}> UnBox Vip</button>
                    </div>
                </div>
            </div>
            <Modal
                                ...
                                <div className='display_flex parent_buybox_header'>
                                    <div className='buybox_modal_header'>UnBox</div>
                                </div>
                                ...
                                <div className='display_flex parent_buybox_header'>
                                    {
                                        textAmount ==="" ?
                                        <button onClick={UnBox} className='button_on_modal yes_button'>Yes</button>
                                        :
                                        <div></div>
                                    }
                ...
                <Modal
                                ...
                                >
                                <div className='display_flex parent_buybox_header'>
                                    <div className='buybox_modal_header'>UnBox Vip</div>
                                </div>
                                <div className='pending_container'>
                                        {
                                            textAmount ==="pending" ? 
                                ...
                                <div className='display_flex parent_buybox_header'>
                                    {
                                        textAmount ==="" ?
                                        <div className='text_question'> Are you sure UnBox Vip?</div> :
                                        <div/>
                                ...
                                <div className='display_flex parent_buybox_header'>
                                    {
                                        textAmount ==="" ?
                                        <button onClick={UnBoxVip} className='button_on_modal yes_button'>Yes</button>
                                        :
                                        <div></div>
                                    }
                                    <button onClick={closeModal} className='button_on_modal close_button'>close</button>
